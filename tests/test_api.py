import pytest
from app import app
from app.user_management import register_user
...
def test_login(client):
    ...
    register_user('testuser@example.com', 'testpassword', 'Test User')
...
def test_update_inventory(client):
    response = client.post('/inventory/update', json={
        'product_id': 'P001',
        'quantity': 10,
...
def test_register_sale(client):
    response = client.post('/sales/register', json={
        'client': 'Cliente 1',
...
def test_register_purchase(client):
    response = client.post('/purchases/register', json={
...
def test_generate_sales_report(client):
    response = client.get('/reports/sales', query_string={
