...
class InventoryManagement:
    ...
    def adjust_inventory(self, product_id, quantity, movement_type):
        ...
        new_stock = current_stock + quantity if movement_type == 'add' else current_stock - quantity
        if new_stock < 0:
            return False
...
def update_inventory(product_id, quantity, movement_type, user, description, reference):
    ...
    if success:
        ...
        VALUES (?, ?, ?, ?, ?, ?, datetime('now'))
        """
        execute_query(query, (product_id, quantity, movement_type, user, description, reference))
    return success
