...
def add_sale_detail(sale_id, product_id, quantity, price_unit, discount):
    ...
    """
    execute_query(
        query, (sale_id, product_id, quantity, price_unit, subtotal, discount)
    )
    update_inventory(
        product_id, -quantity, "Salida", "Sistema", f"Venta {sale_id}", sale_id
    )
...
def get_sale_history(
    client=None, start_date=None, end_date=None, status=None, payment_method=None
):
