...
def update_inventory_route():
    ...
    update_inventory(
        product_id, quantity, movement_type, user, description, reference_document
    )
...
def register_purchase_route():
    ...
    for detail in details:
        ...
        purchase_id = register_purchase(
            product_id, quantity, supplier_id, cost, db
        )  # Asegurarse de que se pase supplier_id
    ...
    return (
        jsonify(
            {"message": "Purchase registered successfully", "purchase_id": purchase_id}
        ),
        201,
    )
