...
def add_purchase_detail(purchase_id, product_id, quantity, price_unit):
    ...
    update_inventory(
        product_id, quantity, "Entrada", "Sistema", f"Compra {purchase_id}", purchase_id
    )
    ...
    if current_status[0] == "Recibida":
        ...
        for detail in details:
            update_inventory(
                ...
                "Salida",
                user,
                f"Cancelación de compra {purchase_id}",
                f"CANCEL-{purchase_id}",
            )
...
def modify_purchase_detail(
    detail_id: int, new_quantity: int = None, new_price: float = None
) -> bool:
