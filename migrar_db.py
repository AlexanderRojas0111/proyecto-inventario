    ...
    with open("data/products.csv", newline="", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            try:
                ...
                print(
                    f"Error: El producto con ID {row['ID_Producto']} ya existe en la base de datos."
                )
    ...
    with open("data/users.csv", newline="", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            try:
                ...
                print(
                    f"Error: El usuario con correo {row['Correo']} ya existe en la base de datos."
                )
    ...
    with open("data/purchases.csv", newline="", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            try:
                ...
                print(
                    f"Error: La columna {e} no se encuentra en el archivo purchases.csv"
                )
            except sqlite3.IntegrityError:
                print(
                    f"Error: La compra con ID {row['ID_Compra']} ya existe en la base de datos."
                )
    ...
    with open("data/sales.csv", newline="", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            try:
                ...
                print(
                    f"Error: La venta con ID {row['ID_Venta']} ya existe en la base de datos."
                )
