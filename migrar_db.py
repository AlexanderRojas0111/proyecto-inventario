...
with open('data/products.csv', newline='', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        try:
            ...
            VALUES (?, ?, ?, ?, ?)
            ''', (row['ID_Producto'], row['Nombre'], row['Stock_Actual'], row['Stock_Minimo'], row['Precio_Unitario']))
...
with open('data/purchases.csv', newline='', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        try:
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (row['ID_Compra'], row['Fecha'], row['Proveedor'], row['Total'], row['Estado'], row['Usuario'], row['Observaciones']))
...
with open('data/sales.csv', newline='', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        try:
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (row['ID_Venta'], row['Fecha'], row['Cliente'], row['Total'], row['Estado'], row['Usuario'], row['Método_Pago'], row['Observaciones']))
        except KeyError as e:
